document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeGame(mode) { gameMode = mode; if (mode === 'pvai') { aiLevel = aiDifficulty.value; } board = setupBoard(); currentPlayer = 'w'; selectedPiece = null; validMoves = []; isGameOver = false; gameOverSoundPlayed = false; castlingRights = { w: { k: true, q: true }, b: { k: true, q: true } }; enPassantTarget = null; lastMove = { from: null, to: null }; mainMenu.classList.add('hidden'); gameContainer.classList.remove('hidden'); stopAllMusic(); playMusic('gameMusic'); renderBoard(); updateStatus(); }
    showMainMenu();

    // --- SOUND SYSTEM ---
    function playSound(soundKey) { if (!isSoundEnabled) return; const sound = sounds[soundKey]; if (sound) { sound.currentTime = 0; sound.play().catch(e => {}); } }
    function playMusic(musicKey) { if (!isSoundEnabled) return; const music = sounds[musicKey]; if (music) { music.currentTime = 0; music.play().catch(e => {}); } }
    ...
    // This is your correct implementation for the home button.
    homeBtn.addEventListener('click', () => {
        // This path is relative to your index.html file.
        // If your game is at '.../games/chess/index.html'
        // and home is at '.../home/home.html', this path is correct.
        window.location.href = '../../home/home.html';
    });
    
    soundToggleBtn.addEventListener('click', () => { isSoundEnabled = !isSoundEnabled; soundToggleBtn.textContent = isSoundEnabled ? '🔊' : '🔇'; if (isSoundEnabled) { if (!gameContainer.classList.contains('hidden')) { playMusic('gameMusic'); } else { playMusic('menuMusic'); } } else { stopAllMusic(); } });
    ...
    function renderBoard() {
        ...
        for (let r = 0; r < 8; r++) {
            for (let c = 0; c < 8; c++) {
                ...
                if (pieceChar) {
                    ...
                    pieceElement.classList.add(getPieceColor(pieceChar) === 'w' ? 'white-piece' : 'black-piece');
        ...
        // FIXED BUG HERE: Attach listeners for both click and touchstart for mobile compatibility
    ...
    function handleSquareClick(event) {
        event.preventDefault(); // FIXED BUG HERE: Prevents "ghost clicks" on mobile where both touch and click fire
    ...
    // --- GAME LOGIC ---
    function selectPiece(r, c) { selectedPiece = { r, c, piece: board[r][c] }; validMoves = getValidMovesForPiece(r, c); renderBoard(); }
    function deselect() { selectedPiece = null; validMoves = []; renderBoard(); }
    function makeMove(from, to) { const piece = board[from.r][from.c]; const capturedPiece = board[to.r][to.c]; if (capturedPiece) { playSound('capture'); } else { playSound('move'); } if (piece.toLowerCase() === 'p' && to.r === enPassantTarget?.r && to.c === enPassantTarget?.c) { const capturedPawnRow = currentPlayer === 'w' ? to.r + 1 : to.r - 1; board[capturedPawnRow][to.c] = null; } board[to.r][to.c] = piece; board[from.r][from.c] = null; if (piece.toLowerCase() === 'k' && Math.abs(from.c - to.c) === 2) { const rookCol = to.c > from.c ? 7 : 0; const newRookCol = to.c > from.c ? 5 : 3; board[from.r][newRookCol] = board[from.r][rookCol]; board[from.r][rookCol] = null; } if (piece.toLowerCase() === 'p' && Math.abs(from.r - to.r) === 2) { enPassantTarget = { r: (from.r + to.r) / 2, c: from.c }; } else { enPassantTarget = null; } const promotionRank = (currentPlayer === 'w') ? 0 : 7; if (piece.toLowerCase() === 'p' && to.r === promotionRank) { if (gameMode === 'pvai' && currentPlayer === 'b') { board[to.r][to.c] = 'q'; finishMove(from, to); } else { showPromotionDialog(from, to); } return; } finishMove(from, to); }
    ...
    function showPromotionDialog(from, to) {
        ...
        promotionDialog.querySelectorAll('button').forEach(button => {
            const handlePromotion = (event) => {
                event.preventDefault(); // FIXED BUG HERE: Make promotion buttons touch-friendly
                const promotionPiece = event.currentTarget.dataset.piece;
            ...
            // FIXED BUG HERE: Listen for both click and touchstart on promotion buttons
            button.addEventListener('click', handlePromotion, { once: true });
            button.addEventListener('touchstart', handlePromotion, { once: true, passive: false });
        });
    }
        ...
        for (let [dr, dc] of knightMoves) {
            ...
            if (isValid(cr, cc)) {
                ...
                if (!targetPiece || getPieceColor(targetPiece) !== color) {
                    // FIXED BUG HERE: It was `c: c`, now it's `c: cc`
    ...
    function findKing(color, boardState = board) {
        ...
        for (let r = 0; r < 8; r++) {
            for (let c = 0; c < 8; c++) {
                // FIXED BUG HERE: It was `boardState[r][a]`, now it's `boardState[r][c]`
