document.addEventListener('DOMContentLoaded', () => {
    ...
    function playMusic(musicKey) { /* ... unchanged ... */ }
    function stopAllMusic() { /* ... unchanged ... */ }
    ...
    soundToggleBtn.addEventListener('click', () => { isSoundEnabled = !isSoundEnabled; soundToggleBtn.textContent = isSoundEnabled ? '🔊' : '🔇'; if (isSoundEnabled) { playSound('uiClick'); if (!gameContainer.classList.contains('hidden')) { playMusic('gameMusic'); } else { playMusic('menuMusic'); } } else { stopAllMusic(); } });

    // --- RENDERING & INTERACTION (The rest of your optimized code is unchanged) ---
    function renderBoard() { /* ... unchanged ... */ }
    function highlightValidMoves() { /* ... unchanged ... */ }
    ...
    function makeMove(from, to) {
        ...
        const isCastling = piece.toLowerCase() === 'k' && Math.abs(from.c - to.c) === 2;

        // MODIFIED FOR SPECIAL SOUNDS
        if (isCastling) {
            playSound('castling');
    ...
    // (finishMove, updateStatus, all move validation, all AI logic, etc.)
