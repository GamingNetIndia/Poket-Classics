document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadSettings() {
        state.isMusicOn = localStorage.getItem('snakeLadderMusic') !== 'false';
    ...
    function showWinPopup() { state.isGameOver = true; playSoundEffect(audio.win); audio.bgMusic.pause(); logMessage(`🎉 Player ${state.currentPlayerIndex + 1} Wins! 🎉`); dom.winMessage.textContent = `Player ${state.currentPlayerIndex + 1} Wins!`; dom.winPopup.classList.remove('hidden'); dom.rollDiceBtn.disabled = true; startConfetti(); }
